...
real Zenith Player ELF executable directly to Zypak. Preserve the existing
...
from `zenithplayer` to `zenithplayer.bin` and writes a shell script at `zenithplayer`.
Electron Builder's Flatpak launcher calls `zypak-wrapper zenithplayer`, so Zypak
...
- an isolated Flatpak target uses `zenithplayer` as the Electron ELF and does not
  apply the custom Linux sandbox wrapper;
- every other supported Linux target keeps the existing `zenithplayer` shell
  wrapper and `zenithplayer.bin` Electron ELF;
...
4. Pristine Flatpak validation inspects `zenithplayer`, while other profiles
   inspect `zenithplayer.bin`.
5. Extracted Flatpak verification reads architecture and checks process
   isolation from `zenithplayer`.
...
Electron executable is named `zenithplayer.bin`.
