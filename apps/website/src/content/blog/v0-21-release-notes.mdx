...
description: Zenith Player v0.21 brings an experimental embedded MPV player on macOS, toggleable sidebars, a smarter command palette, faster loading, and a long list of fixes across M3U, Xtream and Stalker.
featured: true
pubDate: 2026-05-06
author: zenith
heroImage: /zenithplayer/blog/v0-21/wide-changelog.jpg
tags:
    - release
    - release-notes
    - v0.21
...
If Zenith Player is useful to you and you’d like to keep it moving, the easiest way to help is via:

- **GitHub Sponsors** — [github.com/sponsors/zenith](https://github.com/sponsors/zenith)
- **Ko-fi** — [ko-fi.com/zenith](https://ko-fi.com/zenith)
...
The single biggest change in v0.21. macOS builds now ship libmpv inside the app and render it into a Cocoa view embedded in the Zenith Player window — so MPV-quality playback is available without installing anything separately.
...
This was tracked in [#877](https://github.com/hus2005/zenithplayer/issues/877).
...
<BlogImageSlider
    images={[
        ...
        {
            src: '/zenithplayer/blog/v0-21/sidebar-spotlight.webp',
        ...
        {
            src: '/zenithplayer/blog/v0-21/feature-mosaic.webp',
        ...
        {
            src: '/zenithplayer/blog/v0-21/wide-changelog.webp',
...
In **Settings → External player**, select MPV and point Zenith Player at IINA's executable path, for example `/Applications/IINA.app/Contents/MacOS/iina-cli` or `/Applications/IINA.app/Contents/MacOS/IINA`. Do not use `/Applications/IINA.app` directly: Zenith Player only resolves `.app` bundle paths for the real MPV and VLC apps. Streams can launch in IINA, but Zenith Player controls, position polling, and reuse-instance behavior are only guaranteed for MPV IPC.
</Alert>

This also retroactively closes the long-standing request in [#896](https://github.com/hus2005/zenithplayer/issues/896) — IINA support was effectively already there, it just wasn’t documented.
...
- **Manual refresh for file-based playlists** is back. v0.20 silently dropped this for M3U files imported from disk and that was a regression — see [#891](https://github.com/hus2005/zenithplayer/issues/891). It’s back, and the file path is now preserved correctly across refreshes.
- **VLC reuse-instance setting.** VLC was unconditionally spawning a new window per stream because its built-in single-instance preference doesn’t play nicely with how Zenith Player launches it. There’s now an opt-in setting that mirrors the existing MPV “reuse instance” behavior — one VLC window, controlled via its RC interface — see [#893](https://github.com/hus2005/zenithplayer/issues/893).
...
- **Sort “All Channels”** by name, or keep the original playlist order — handy for huge lists where alphabetical isn’t always what you want. This is a partial answer to the long-running [#738](https://github.com/hus2005/zenithplayer/issues/738).
...
<LinkCards
    links={[
        ...
        },
        {
            ...
            hint: 'Browse every Zenith Player release.',
            icon: 'github',
        },
    ]}
/>
